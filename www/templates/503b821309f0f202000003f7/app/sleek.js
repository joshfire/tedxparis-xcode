...
function (Collection, DynamicContainer, Item, List, CardPanel, FadeInPanel, FactoryMedia, ImageLoader, ImagesLoader, Router, Backbone, _, $, ImageGallery) {
  ...
  _.extend(Sleek.prototype, {
    ...
    initialize: function() {
      ...
      $("header a, #toolbar a").live("touchstart mousedown",fastNavigate);
      // $("#viewport a").live("tap click",fastNavigate);
      ...
      this.setColor(Joshfire.factory.config.template.options.color || 'gray', function () {
        ...
        _.forEach(datasources, _.bind(function(datasource, index) {
          ...
          // >>> TEDxParis Fix : force video outputtype on union operator
          if(typeof datasource.config.outputType === 'undefined' && datasource.config.name === 'Videos') {
            datasource.config.outputType = 'VideoObject';
          }
          // <<<
    ...
    createDetailContainer: function(section, isSingle) {
      ...
      return new DynamicContainer({
        viewFactory: function (options) {
          ...
          if (options.model && options.model.get("itemType")=="BlogPosting") {
            options.model.attributes.articleBody = options.model.get("articleBody").replace(/\n\s*\n/g,"<br/>");
          }
    ...
    itemFactory: function(section) {
      ...
      return function(model, offset) {
        ...
        madeItem.bind("load",function() {
          if (typeof cordova!="undefined") cordova.exec(null, null, "SplashScreen", "hide", []);
          //console.log("X",madeItem.el);
          //$(".fadein",madeItem.el).removeClass("fadein");
        });
